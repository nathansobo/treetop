...
context "A new failure tree whose root is at a given failure index" do
  ...
  specify "reports its own index as its highest subtree index" do
    @failure_tree.max_subtree_index.should == @index
  end
end

context "A new failure tree instantiated a single subtree that may have caused the failure leading to the production of this tree or one of its ancestors parent" do
  setup do
    @expression = mock('expression producing the failure tree')
    @index = 68
  ...
  specify "makes it available as one of its subtrees" do
    subtrees = @failure_tree.subtrees
    subtrees.size.should == 1
    subtrees.first.should == @subtree
  end
  
  specify "reports the index of its subtree as the highest subtree index" do
    @failure_tree.max_subtree_index.should == @subtree.index
  end
end

context "A new failure tree instantiated with two subtrees, two with max subtree index index of 8 and one at index 3" do
  setup do
    @expression = mock('expression producing the failure tree')
    @index = 1
  ...
  specify "keeps only the subtrees with the highest subtree indices as its subtrees" do
    subtrees = @failure_tree.subtrees
  ...
  specify "reports 8 as its highest subtree index" do
