    ...
    if !OPTIMIZE
      def parse_at(input, start_index, parser)
        if input.index(prefix, start_index) == start_index
          return node_class.new(self, input, start_index...(prefix.length + start_index))
        else
      ...
      inline do |builder|
        ...
        builder.c <<-C
          VALUE parse_at(VALUE input, int start_index, VALUE parser) {
            ...
            VALUE parse_failure_argv[2], node_class_argv[3];
            VALUE node_class;
            ...
            node_class_argv[0] = self;
            node_class_argv[1] = input;
            node_class_argv[2] = rb_range_new(INT2NUM(start_index), INT2NUM(prefix_length + start_index), 1);
            return (rb_class_new_instance(3, node_class_argv, node_class));
          }
        C
      end
    end
